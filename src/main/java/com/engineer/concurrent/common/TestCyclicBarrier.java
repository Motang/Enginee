...
public class TestCyclicBarrier {
	...
	public class Grouper implements Runnable {
		...
		public void run() {
			...
			// 30. 使用 results 对象的 getData()
			// 方法来获得每行数字出现的次数。然后，处理array的全部元素，把每个元素的值加给 finalResult 变量。
	...
	public static void main(String[] args) {
		...
		MatrixMock mock = test.new MatrixMock(ROWS, NUMBERS, SEARCH);

		// 35. 创建 Results 对象，名为 results。它将有 10,000 元素。
		Results results = test.new Results(ROWS);

		// 36. 创建 Grouper 对象，名为 grouper。
		Grouper grouper = test.new Grouper(results);
		...
		for (int i = 0; i < PARTICIPANTS; i++) {
			searchers[i] = test.new Searcher(i * LINES_PARTICIPANT, (i * LINES_PARTICIPANT)
